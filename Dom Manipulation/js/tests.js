...
(function() {
  ...
  appendAndAddAttras(body, testBtn, [['id', 'testBtn']]);

  const testMdl = createEl('DIV');
  appendAndAddAttras(body, testMdl, [['id', 'testMdl']]);

  const mdlH2 = createEl('H2');
  mdlH2.textContent = 'Results';
  appendAndAddAttras(testMdl, mdlH2, [['id', 'mdlH1']]);

  const mdlMsg = createEl('OL');
  appendAndAddAttras(testMdl, mdlMsg, [['id', 'mdlMsg']]);

  const mdlClose = createEl('BUTTON');
  mdlClose.textContent = 'Close';
  appendAndAddAttras(testMdl, mdlClose, [['id', 'mdlClose']]);
  /* End Section */
  ...
  const domCheck = (el, id) => {
    ...
    if (el) {
      message = `The '${el.tagName}'' with id of '${id}' is present.`;
  ...
  const attraCheck = (el, attra, val) => {
    ...
    if (el.getAttribute(attra) === val) {
      message = `The '${el.id}' ${el.tagName} element has a '${attra}' property set to '${val}'.`;
      mdlMsgItm.style.color = 'black';
    } else {
  ...
  const textCheck = (el, text) => {
    ...
    if (el.innerHTML === text) {
      message = `The '${el.tagName}' element has a its text set to '${text}'.`;
      mdlMsgItm.style.color = 'black';
    } else {
